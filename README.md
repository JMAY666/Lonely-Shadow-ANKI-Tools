...
新增 **逐空标记与薄弱项复习**：复习底栏「逐空」管理本轮标记；答案旁的 ✓ 可标记或重新加入复习。支持标准文字挖空和已适配的思维导图 V3 网页 SVG 矩形遮挡，同轮再现只遮挡未记住项，支持完整测试、重置、撤销和重启恢复。标记仅保存在当前账户本机，实际毕业后下次正常间隔复习重新完整测试；原评分、调度、同步与学习记录保持原义。支持范围和轮次规则见 [逐空复习说明](docs/WEAK-REVIEW.md)。
...
新增 **双栏复习**：从 SynapsePro 左侧快捷栏开启，两栏独立选牌组、翻面和评分，共用原调度与学习记录；支持重叠牌组避让、激活栏快捷键与音频、全局撤销、拖动栏宽和窄窗标签切换。新版启动包为 `dist/Anki-weak-review-26.8.1/Anki.exe`，操作、兼容范围和验证命令见 [双栏复习说明](docs/DUAL-REVIEW.md)。

新增内置 **Minimize to Tray 2 0.2** 与 **AnkiPenDown 1.1**，入口为「SynapsePro 左侧快捷栏 → 托盘与手写」。按用户确认，默认点击 × 正常退出，可按需开启托盘隐藏；卡片手写保留双画笔、荧光笔、笔画橡皮、撤销与画布设置，设置按账户保存，菜单与控件已接入简体中文。实际使用版本更新在 `dist/Anki-weak-review-26.8.1/Anki.exe`。来源、配置兼容、验证和回退见 [托盘与手写说明](docs/DESKTOP-TOOLS.md)。
...
本机实际使用版本：`dist/Anki-weak-review-26.8.1/Anki.exe`。无需单独安装插件；从牌组直接学习，从顶部「统计」查看数据与报告。
