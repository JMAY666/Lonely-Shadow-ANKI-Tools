...
新增 **逐空标记与薄弱项复习**：复习底栏「逐空」管理本轮标记；答案旁的 ✓ 可标记或重新加入复习，鼠标点击后可直接按空格继续显示答案。支持标准文字挖空、Anki Studio 灰色遮挡、Enhanced Cloze 粉色填空，以及已适配的思维导图 V3 网页点击文字填空、表格单元格填空和 SVG 矩形遮挡，同轮再现只遮挡未记住项，支持完整测试、重置、撤销和重启恢复。标记仅保存在当前账户本机，实际毕业后下次正常间隔复习重新完整测试；原评分、调度、同步与学习记录保持原义。支持范围和轮次规则见 [逐空复习说明](docs/WEAK-REVIEW.md)。
